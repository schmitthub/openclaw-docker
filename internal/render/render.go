...
# Seed openclaw.json if not already present in host config dir.
OPENCLAW_JSON_SEED="$ROOT_DIR/compose/openclaw/openclaw.json"
OPENCLAW_JSON_TARGET="$HOST_CONFIG_DIR/openclaw.json"
...
func openClawJSONContent(opts Options) string {
	...
	if opts.ExternalOrigin != "" {
		origins = append(origins, fmt.Sprintf("%q", opts.ExternalOrigin))
	}
...
`, gatewayPort, strings.Join(origins, ", "))
}
